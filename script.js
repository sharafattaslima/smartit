// ==UserScript==
// @name         SMARTIT IVAC MASTER PRO (@S) - CapSolver Edition V2.4
// @namespace    smartit.ivac.pro.v2.capsolver
// @version      2.4
// @description  Sign-In/OTP Auto-Retry, Auto OTP Paste, Web Autofill, CapSolver Auto Captcha Bypass (With Token Caching)
...
(function() {
    ...
    const SETTINGS = {
        ...
        apiMinDelay: 1000,
        apiMaxDelay: 2800,
        fastMin: 2500, fastMax: 5000,
        stealthMin: 12000, stealthMax: 22000
    };

    // --- মেমরি রিস্টোর (Persistent State) ---
    ...
    // Track globally which button is currently glowing
    let activeGlowBtnId = null;

    // --- CAPTCHA CACHING VARIABLES ---
    let globalCaptchaToken = "";
    let globalCaptchaTimestamp = 0;
    const CAPTCHA_LIFETIME = 110000; // 110 seconds (Just under 2 minutes validity)

    let existingToken = localStorage.getItem('token');
    if (!existingToken) {
        try {
            let authStore = JSON.parse(localStorage.getItem('auth-storage'));
            if (authStore && authStore.state && authStore.state.token) existingToken = authStore.state.token;
        } catch(e) {}
    }
    ...
    let signInRetryTimeout = null;
    let otpRetryTimeout = null;
        ...
        fetch(window.location.origin, { method: 'HEAD', cache: 'no-store' }).then(res => {
            try {
                ...
                if (dateHeader) {
                    ...
                    sessionStorage.setItem('ivac_time_offset', timeOffset);
                }
            } catch(e) {}
        }).catch(()=>{});
    }
    syncIVACServerTime();
    window.addEventListener('load', syncIVACServerTime);
    ...
    function getBDTime() {
        return new Date(Date.now() + timeOffset).toLocaleTimeString('en-US', { timeZone: 'Asia/Dhaka', hour12: true });
    }

    // --- ২. স্টাইল ---
    GM_addStyle(`
        #ivac-hybrid-panel { position: fixed; top: 15px; right: 15px; width: 420px; background: #ffffff; border-radius: 16px; box-shadow: 0 25px 50px -12px rgba(0,0,0,0.4), 0 0 0 1px rgba(0,0,0,0.05); z-index: 999999999; font-family: 'Inter', 'Segoe UI', system-ui, sans-serif; color: #1e293b; transition: box-shadow 0.3s ease; overflow: hidden; min-width: 330px; min-height: 400px; display: flex; flex-direction: column; cursor: grab; }
        ...
        /* FIXED: Added fixed height to force scrollbar */
        #live-log { flex: none; height: 180px; min-height: 120px; max-height: 250px; background: #0f172a; border-radius: 10px; padding: 12px; overflow-y: auto; color: #4ade80; font-size: 11px; border: 1px solid #1e293b; margin-bottom: 12px; font-family: 'Fira Code', Consolas, monospace; word-wrap: break-word; box-shadow: inset 0 2px 10px rgba(0,0,0,0.5); line-height: 1.5; cursor: text; }
        ...
        .g-blue { background: linear-gradient(180deg, #3b82f6, #1d4ed8); }
        .g-green { background: linear-gradient(180deg, #10b981, #047857); }
        .g-purple { background: linear-gradient(180deg, #a855f7, #6d28d9); }
        .g-orange { background: linear-gradient(180deg, #f97316, #c2410c); }
        .g-cyan { background: linear-gradient(180deg, #06b6d4, #0e7490); }
        .g-red { background: linear-gradient(180deg, #ef4444, #b91c1c); }
        ...
        .dot-green { background: #34d399 !important; box-shadow: 0 0 8px rgba(52,211,153,0.6) !important; }
        .dot-yellow { background: #fbbf24 !important; box-shadow: 0 0 8px rgba(251,191,36,0.6) !important; }
        ...
        .ui-input-small:hover { border-color: #cbd5e1; }
        .ui-input-small:focus { border-color: #3b82f6; background: #ffffff; box-shadow: 0 0 0 4px rgba(59,130,246,0.15); transform: translateY(-1px); }
        ...
        .ui-input-otp:hover { border-color: #cbd5e1; }
        .ui-input-otp:focus { border-color: #10b981; background: #ffffff; box-shadow: 0 0 0 4px rgba(16,185,129,0.15), inset 0 2px 4px rgba(0,0,0,0.03); transform: translateY(-1px); }
        
        @keyframes api-working-pulse {
            0% { box-shadow: 0 6px 12px rgba(0,0,0,0.25), inset 0 4px 6px rgba(255,255,255,0.4), inset 0 -4px 6px rgba(0,0,0,0.2), 0 0 5px rgba(234, 179, 8, 0.6); }
            50% { box-shadow: 0 6px 12px rgba(0,0,0,0.25), inset 0 4px 6px rgba(255,255,255,0.4), inset 0 -4px 6px rgba(0,0,0,0.2), 0 0 20px 8px rgba(234, 179, 8, 1); transform: scale(1.02); }
            100% { box-shadow: 0 6px 12px rgba(0,0,0,0.25), inset 0 4px 6px rgba(255,255,255,0.4), inset 0 -4px 6px rgba(0,0,0,0.2), 0 0 5px rgba(234, 179, 8, 0.6); }
        }
        ...
        * { user-select: text !important; -webkit-user-select: text !important; }
        ...
        .ivac-toast.error { border-color: #f87171; }
        .ivac-toast.success { background: #f0fdf4; border-color: #4ade80; }
        .ivac-toast-left { display: flex; align-items: center; gap: 14px; }
        .ivac-toast-icon { display: flex; align-items: center; justify-content: center; }
        .ivac-toast-text { display: flex; flex-direction: column; }
        .ivac-toast-title { font-weight: 600; font-size: 16px; margin-bottom: 2px; }
        .ivac-toast.error .ivac-toast-title, .ivac-toast.error .ivac-toast-msg { color: #7f1d1d; }
        .ivac-toast.success .ivac-toast-title, .ivac-toast.success .ivac-toast-msg { color: #14532d; }
        .ivac-toast-msg { font-size: 14px; font-weight: 500; opacity: 0.9; }
        .ivac-toast-close { cursor: pointer; padding: 4px; display: flex; align-items: center; opacity: 0.6; transition: opacity 0.2s; background: none; border: none; }
        .ivac-toast-close:hover { opacity: 1; }
        .ivac-toast.error .ivac-toast-close svg { stroke: #7f1d1d; }
        .ivac-toast.success .ivac-toast-close svg { stroke: #14532d; }
        ...
        @keyframes toastFadeOut { from { opacity: 1; transform: translateY(0) scale(0.95); } to { opacity: 0; transform: translateY(-10px) scale(0.95); } }
    `);

    // --- CAPSOLVER & API HELPERS ---
    function fetchGM(url, options) {
        return new Promise((resolve, reject) => {
            // উইন্ডো অবজেক্ট বা সরাসরি কনটেক্সট থেকে পারমিশন নেওয়া
            const gmXHR = typeof GM_xmlhttpRequest !== 'undefined' ? GM_xmlhttpRequest : (window.GM_xmlhttpRequest || (window.GM && window.GM.xmlHttpRequest));
            
            if (!gmXHR) {
                addLog("❌ Error: GM_xmlhttpRequest is not available! Cross-origin request failed.", "error");
                reject(new Error("GM_xmlhttpRequest not found"));
                return;
            }
    ...
    function getRecaptchaSiteKey() {
        ...
        let iframe = document.querySelector('iframe[src*="recaptcha/api2/anchor"]');
        if (iframe) {
            let match = iframe.src.match(/k=([^&]+)/);
            if (match) return match[1];
        }
        let script = document.querySelector('script[src*="recaptcha/api.js?render="]');
        if (script) {
            let match = script.src.match(/render=([^&]+)/);
            if (match) return match[1];
        }
        return null;
    }
    ...
    async function autoSolveCaptcha(apiKey, siteKey, pageUrl) {
        try {
            let createRes = await fetchGM("https://api.capsolver.com/createTask", {
                method: "POST", headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    clientKey: apiKey,
                    task: { type: "ReCaptchaV2TaskProxyless", websiteURL: pageUrl, websiteKey: siteKey, isInvisible: false }
                })
            });
            ...
            addLog(`🤖 CapSolver Task: ${taskId}... Waiting for result`, "info");

            for (let i = 0; i < 60; i++) {
                await new Promise(r => setTimeout(r, 2000));
                let pollRes = await fetchGM("https://api.capsolver.com/getTaskResult", {
                    method: "POST", headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({ clientKey: apiKey, taskId: taskId })
                });
                let pollData = await pollRes.json();
                if (pollData.status === "ready") {
                    addLog("✅ CapSolver Solved Captcha Successfully!", "success");
                    return pollData.solution.gRecaptchaResponse;
                } else if (pollData.status === "failed" || pollData.errorId !== 0) {
                    addLog(`❌ CapSolver Failed: ${pollData.errorDescription || 'Unknown Error'}`, "error");
                    return null;
                }
            }
    ...
    function setPersistentGlow(btnId) {
        if (activeGlowBtnId && activeGlowBtnId !== btnId) {
            let oldBtn = document.getElementById(activeGlowBtnId);
            if (oldBtn) oldBtn.classList.remove('btn-highlight');
        }
        activeGlowBtnId = btnId;
        let newBtn = document.getElementById(btnId);
        if (newBtn) newBtn.classList.add('btn-highlight');
    }
    ...
    function showToast(title, message, type = 'error') {
        ...
        if (!container) {
            ...
            container.id = 'ivac-toast-container';
            document.body.appendChild(container);
        }
        const toast = document.createElement('div');
        toast.className = `ivac-toast ${type}`;
        let iconSvg = type === 'error' ?
            `<svg viewBox="0 0 24 24" width="28" height="28" fill="none" stroke="#ef4444" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10" fill="#fee2e2"></circle><line x1="15" y1="9" x2="9" y2="15"></line><line x1="9" y1="9" x2="15" y2="15"></line></svg>` :
            `<svg viewBox="0 0 24 24" width="28" height="28" fill="none" stroke="#22c55e" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10" fill="#dcfce7"></circle><polyline points="20 6 9 17 4 12"></polyline></svg>`;
        toast.innerHTML = `
            <div class="ivac-toast-left">
                <div class="ivac-toast-icon">${iconSvg}</div>
                <div class="ivac-toast-text">
                    <span class="ivac-toast-title">${title}</span>
                    <span class="ivac-toast-msg">${message}</span>
                </div>
            </div>
            <button class="ivac-toast-close" title="Close">
    ...
    function interceptApiCall(url) {
        if (!url) return false;
        return url.includes('/slots/reserveSlot') || url.includes('/auth/signin') || url.includes('/otp/verifySigninOtp') || url.includes('/file/payment-amount');
    }

    function formatPreview(data) {
        try {
            let clean = JSON.parse(JSON.stringify(data));
            if (clean.data && clean.data.accessToken) clean.data.accessToken = "[HIDDEN]";
            if (clean.accessToken) clean.accessToken = "[HIDDEN]";
            if (clean.data && clean.data.token) clean.data.token = "[HIDDEN]";
            if (clean.data && clean.data.refreshToken) clean.data.refreshToken = "[HIDDEN]";
            return JSON.stringify(clean, null, 2);
        } catch(e) { return JSON.stringify(data); }
    }

    function processServerReply(data, status, url) {
        let uiMsg = `Status: ${status}`;
        let rawLogMsg = "";
        if (typeof data === 'string') {
            if (data.includes('503') || data.toLowerCase().includes('service unavailable') || data.toLowerCase().includes('cloudflare')) uiMsg = "503 Server Busy / Cloudflare";
            else if (data.includes('502') || data.toLowerCase().includes('bad gateway')) uiMsg = "502 Bad Gateway";
            else if (data.includes('504') || data.toLowerCase().includes('gateway timeout')) uiMsg = "504 Gateway Timeout";
            else uiMsg = `HTML/Text Error (${status})`;
            rawLogMsg = data.length > 250 ? data.substring(0, 250) + '...[TRUNCATED HTML]' : data;
        }
        else if (data) {
            if (data.message) uiMsg = data.message;
            else if (data.error) uiMsg = typeof data.error === 'string' ? data.error : JSON.stringify(data.error);
            else if (typeof data.data === 'string') uiMsg = data.data;
            else if (data.status) uiMsg = data.status;
            else if (data.statusCode) uiMsg = `Error: ${data.statusCode}`;
            rawLogMsg = formatPreview(data);
        }
        sessionStorage.setItem('ivac_last_reply', uiMsg);
        const elResp = document.getElementById('detail-response');
        if (elResp) { elResp.innerText = uiMsg; }
    ...
    window.fetch = async function(...args) {
        ...
        let isTarget = interceptApiCall(url);
        if (isTarget) {
            apiHitCount++; sessionStorage.setItem('ivac_api_hits', apiHitCount);
            const elHits = document.getElementById('detail-api-hits'); if (elHits) elHits.innerText = apiHitCount;
            const elResp = document.getElementById('detail-response'); if (elResp) elResp.innerText = 'Waiting for reply...';
        }
        try {
            ...
            if (isTarget) {
                ...
                clone.text().then(text => {
                    try { processServerReply(JSON.parse(text), response.status, url); }
                    catch(e) { processServerReply(text, response.status, url); }
                }).catch(()=>{});
            }
            return response;
        } catch(e) {
            if (isTarget) {
                const elResp = document.getElementById('detail-response');
                if (elResp) elResp.innerText = 'Network/Connection Error';
                addLog(`📥 [API FETCH FAILED]: Code 0 | ${e.toString()}`, "error");
            }
            throw e;
        }
    };
    ...
    XMLHttpRequest.prototype.send = function() {
        let isTarget = this._url && (this._url.includes('/signin') || this._url.includes('/verifySigninOtp') || this._url.includes('/reserveSlot') || this._url.includes('/get-booking-config'));
        if (isTarget) {
            apiHitCount++; sessionStorage.setItem('ivac_api_hits', apiHitCount);
            const elHits = document.getElementById('detail-api-hits'); if (elHits) elHits.innerText = apiHitCount;
            const elResp = document.getElementById('detail-response'); if (elResp) elResp.innerText = 'Waiting for reply...';
            this.addEventListener('load', function() {
                try {
                    let dateHeader = this.getResponseHeader('Date');
                    if (dateHeader) {
                        timeOffset = new Date(dateHeader).getTime() - Date.now();
                        sessionStorage.setItem('ivac_time_offset', timeOffset);
                    }
                } catch(e) {}
                try { processServerReply(JSON.parse(this.responseText), this.status, this._url); }
                catch(e) { processServerReply(this.responseText, this.status, this._url); }
            });
        }
    ...
    function setupDrag(panel) {
        ...
        panel.onmousedown=(e)=>{
            const t=e.target;
            if(!t || ['INPUT', 'TEXTAREA', 'BUTTON', 'A'].includes(t.tagName) || t.isContentEditable || t.closest('input') || t.closest('button') || t.closest('#live-log') || t.closest('#detail-response') || (typeof t.className==='string' && t.className.includes('resizer')) || t.closest('.resizer-br') || t.closest('.resizer-bl')) {
                return;
            }
            e.preventDefault();
    ...
    function addLog(msg, type = 'info') {
        ...
        box.prepend(div);
        if (box.childNodes.length > 50) box.lastChild.remove();
    ...
    // ==================== PERFECT NAVIGATION FIX ====================
    window.safeNavigateToPayment = function() {
        try {
            let popup = document.getElementById('success-payment-popup');
            if(popup) popup.remove();
            window.location.href = '/appointment/continue-payment';
        } catch(e) {
            window.location.href = '/appointment/continue-payment';
        }
    };
    ...
    function showSuccessPopup(resId) {
        ...
        popup.id = 'success-payment-popup';
        popup.innerHTML = `
            <div style="position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: rgba(15,23,42,0.85); z-index: 99999999999; display: flex; justify-content: center; align-items: center; backdrop-filter: blur(8px);">
                <div style="background: #ffffff; padding: 40px; border-radius: 24px; text-align: center; max-width: 450px; box-shadow: 0 25px 50px -12px rgba(16,185,129,0.6); border: 4px solid #10b981;">
                    ...
                    <div style="background: #f1f5f9; padding: 15px; border-radius: 12px; margin-bottom: 25px; word-break: break-all; font-family: 'Fira Code', monospace; font-size: 13px; color: #0f172a; border: 1px dashed #cbd5e1;">
                        <strong style="color: #3b82f6;">Reservation ID:</strong><br>
                        ${resId || 'Check Profile/Dashboard'}
                    </div>
                    <button onclick="window.safeNavigateToPayment()" style="display: block; width: 100%; padding: 16px; background: linear-gradient(135deg, #10b981, #059669); color: white; border: none; font-size: 18px; font-weight: bold; font-family: 'Inter', sans-serif; border-radius: 12px; transition: all 0.2s ease; box-shadow: 0 10px 15px -3px rgba(16,185,129,0.4); cursor: pointer;" onmouseover="this.style.transform='scale(1.05)'" onmouseout="this.style.transform='scale(1)'">
                        💳 CLICK HERE TO PAY
                    </button>
                    <button onclick="document.getElementById('success-payment-popup').remove()" style="margin-top: 20px; background: transparent; border: none; color: #94a3b8; font-size: 14px; font-weight: bold; cursor: pointer; text-decoration: underline; font-family: 'Inter', sans-serif;">Close this popup</button>
                </div>
            </div>
        `;
    ...
    async function pureApiSignIn() {
        ...
        let cToken = document.querySelector('[name="g-recaptcha-response"]')?.value || document.querySelector('input[name="captchaToken"]')?.value;

        // --- CACHED CAPTCHA LOGIC ---
        if (!cToken || cToken.length < 20) {
            if (globalCaptchaToken && (Date.now() - globalCaptchaTimestamp < CAPTCHA_LIFETIME)) {
                cToken = globalCaptchaToken;
                addLog("♻️ Using Cached CapSolver Token...", "info");
            } else if (capKey) {
                ...
                if(siteKey) {
                    ...
                    if (cToken) {
                        globalCaptchaToken = cToken;
                        globalCaptchaTimestamp = Date.now();
                    }
                } else {
        ...
        try {
            const response = await fetch("https://api.ivacbd.com/iams/api/v1/auth/signin", {
                ...
                body: JSON.stringify({ phone: phoneVal, password: passVal, captchaToken: cToken })
            });
            if (response.status >= 500) throw new Error("Server 5xx Error");
            const result = await response.json();
            if (response.ok && result.successFlag && result.data?.accessToken) {
                authToken = result.data.tokenType + " " + result.data.accessToken; currentRequestId = result.data.requestId; localStorage.setItem('ivac_req_id', currentRequestId);
                checkAuthStatus(); addLog("✅ [API] LOGIN SUCCESS! Check SMS for OTP.", "success"); updateLiveDetails('status', 'Waiting for OTP');
                showToast("Success", "Sign-in successful. Check your SMS for OTP.", "success");
            } else {
                updateLiveDetails('status', 'Sign-In Failed'); addLog("🚫 Sign-In Failed: Wrong Password or Captcha Expired.", "error");
                showToast("Error", "Sign-In Failed. Wrong Password or Captcha Expired.", "error");
            }
        } catch (e) {
            if (!automationActive) { addLog("🛑 Auto-Retry cancelled (STOP ALL is active).", "warning"); return; }
            let delay = Math.floor(Math.random() * (3000 - 1000 + 1)) + 1000;
            updateLiveDetails('status', `5xx/Error - Retry in ${(delay/1000).toFixed(1)}s`);
            addLog(`⚠️ Server Busy. Auto-Retrying Sign-In in ${(delay/1000).toFixed(1)}s...`, "warning");
            signInRetryTimeout = setTimeout(pureApiSignIn, delay);
        } finally {
            if (signInBtn) {
                signInBtn.disabled = false;
                signInBtn.style.opacity = '1';
                signInBtn.style.transform = '';
                signInBtn.innerHTML = origText;
            }
        }
    }
        ...
        try {
            ...
            if (!code) {
                try {
                    ...
                    if (clipText.length >= 6 && clipText.length <= 7) { code = clipText; document.getElementById('ui-otp').value = code; }
                } catch(e) {}
            }
            if (!code) {
                addLog("⚠️ OTP Box is blank! Please enter OTP.", "error");
                showToast("Error", "Please enter your OTP.", "error");
                return;
            }
            if (!/^\d{6,7}$/.test(code)) {
                addLog("⚠️ Invalid OTP! Must be 6 or 7 digits.", "error");
                showToast("Error", "Invalid OTP format.", "error");
                return;
            }
            if (!authToken || !currentRequestId) {
                addLog("⚠️ Missing Request ID! Please hit Sign-In first.", "error");
                showToast("Error", "Missing Request ID! Hit Sign-In first.", "error");
                return;
            }

            // TRIGGER PERSISTENT HIGHLIGHT GLOW
            setPersistentGlow('btn-verify');
            const verifyBtn = document.getElementById('btn-verify');
            let origText = "VERIFY OTP";
            if (verifyBtn) {
                origText = verifyBtn.dataset.origText || verifyBtn.innerText || origText;
            ...
            const response = await fetch("https://api.ivacbd.com/iams/api/v1/otp/verifySigninOtp", {
                method: "POST", headers: { "Accept": "application/json", "Content-Type": "application/json", "Authorization": authToken },
                body: JSON.stringify({ requestId: currentRequestId, phone: document.getElementById('ui-phone').value || SETTINGS.phone, code: code, otpChannel: "PHONE" })
            });
            if (response.status >= 500) throw new Error("Server 5xx Error");
            const result = await response.json();
            if (response.ok && result.successFlag && result.data?.verified) {
                checkAuthStatus(); addLog(`🎯 [API] OTP VERIFIED! Injecting Token...`, "success"); updateLiveDetails('status', 'OTP Verified. Injecting...');
                showToast("Success", "OTP Verified Successfully!", "success");
                const rawToken = authToken.replace('Bearer ', '');
                localStorage.setItem('token', rawToken); sessionStorage.setItem('token', rawToken);
                let authObj = { state: { token: rawToken }, version: 0 };
                try {
                    let parsed = JSON.parse(localStorage.getItem('auth-storage'));
                    if (parsed && typeof parsed === 'object') {
                        if (!parsed.state) parsed.state = {};
                        parsed.state.token = rawToken;
                        authObj = parsed;
                    }
                } catch(e) {}
                localStorage.setItem('auth-storage', JSON.stringify(authObj));
                localStorage.removeItem('ivac_req_id'); currentRequestId = null;
                updateLiveDetails('status', 'Routing to Dashboard'); setTimeout(() => { window.location.href = '/appointment/waiting'; }, 300);
            } else {
                updateLiveDetails('status', 'OTP Failed'); addLog("🚨 Invalid or Expired OTP! Stopped Auto-Retry.", "error");
            ...
            let delay = Math.floor(Math.random() * (3000 - 1000 + 1)) + 1000;
            updateLiveDetails('status', `5xx/Error - Retry in ${(delay/1000).toFixed(1)}s`);
            addLog(`⚠️ Server Busy. Auto-Retrying OTP in ${(delay/1000).toFixed(1)}s...`, "warning");
            otpRetryTimeout = setTimeout(pureApiVerifyOTP, delay);
        } finally {
            const verifyBtn = document.getElementById('btn-verify');
            if (verifyBtn) {
    ...
    function autoSelectDateTime() {
        ...
        document.querySelectorAll('div, span, button, .cursor-pointer').forEach(el => {
            ...
            if (!txt || el.disabled || el.classList.contains('opacity-50') || el.classList.contains('cursor-not-allowed')) return;
            if (/\d{2}-\d{2}-\d{4}/.test(txt) || /\b(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\b/i.test(txt)) { dates.push(el); }
            else if (/^\d{1,2}:\d{2}\s*(AM|PM)?\s*-\s*\d{1,2}:\d{2}\s*(AM|PM)?$/.test(txt)) { times.push(el); }
        });
        if (times.length > 0) { updateLiveDetails('slots', `${times.length} Slot(s) Available!`); document.getElementById('detail-slots').style.color = '#059669'; }
        else { updateLiveDetails('slots', 'Searching...'); document.getElementById('detail-slots').style.color = '#dc2626'; }
        if (times.length > 0) { let lastTime = times[times.length - 1]; let timeStr = lastTime.innerText.split('\n')[0].trim(); updateLiveDetails('time', timeStr); if (memoryLastTime !== timeStr) { lastTime.click(); memoryLastTime = timeStr; addLog(`🖱️ Auto-clicked Time Slot [${timeStr}]`, 'info'); } }
        if (dates.length > 0) { let lastDate = dates[dates.length - 1]; let dateStr = lastDate.innerText.trim(); updateLiveDetails('date', dateStr); if (memoryLastDate !== dateStr) { lastDate.click(); setTimeout(() => { if(document.body.contains(lastDate)) lastDate.click(); }, 100); memoryLastDate = dateStr; addLog(`📅 Auto-clicked Date (2x) [${dateStr}]`, 'info'); } }
    }

    function handleCaptchaPopup() {
        let iframe = document.querySelector('iframe[title*="recaptcha"]');
        if (iframe) { let container = iframe.closest('.g-recaptcha') || iframe.parentElement.parentElement; if (container && !container.classList.contains('custom-captcha-popup')) { container.classList.add('custom-captcha-popup'); addLog("🚨 Captcha Pop-up Active! Solve it manually.", "warning"); updateLiveDetails('status', 'Waiting for Captcha...'); } }
    }
    function removeCaptchaPopup() { let container = document.querySelector('.custom-captcha-popup'); if (container) container.classList.remove('custom-captcha-popup'); }

    async function fireReserveSlotAPI(solvedCaptchaToken, isManual = false) {
        if (isReservingApi && !isManual) return;
        isReservingApi = true;
        const rawToken = localStorage.getItem('token');
        if (!rawToken) { isReservingApi = false; addLog("ERR: No Auth Token found!", "error"); return; }

        const capKey = document.getElementById('ui-capsolver').value.trim();

        // --- CACHED CAPTCHA LOGIC ---
        if (!solvedCaptchaToken || solvedCaptchaToken.length < 20) {
            if (globalCaptchaToken && (Date.now() - globalCaptchaTimestamp < CAPTCHA_LIFETIME)) {
                solvedCaptchaToken = globalCaptchaToken;
                addLog("♻️ Using Cached CapSolver Token for Booking...", "info");
            } else if (capKey) {
                let siteKey = getRecaptchaSiteKey();
                if(siteKey) {
                    updateLiveDetails('status', 'Solving Captcha...');
                    ...
                    if (solvedCaptchaToken) {
                        globalCaptchaToken = solvedCaptchaToken;
                        globalCaptchaTimestamp = Date.now();
                    }
                }
            }
        ...
        if (!solvedCaptchaToken || solvedCaptchaToken.length < 20) {
            ...
            addLog("⚠️ Missing Captcha Token for Reservation!", "error");
            showToast("Error", "Captcha missing!", "error");
            return;
        }

        // TRIGGER PERSISTENT HIGHLIGHT GLOW
        setPersistentGlow('btn-api-reserve');
        const reserveBtn = document.getElementById('btn-api-reserve');
        let origText = "CONTINUE BOOKING";
        if (reserveBtn) {
            origText = reserveBtn.dataset.origText || reserveBtn.innerText || origText;
            reserveBtn.disabled = true;
            reserveBtn.style.opacity = '0.7';
            reserveBtn.style.transform = 'translateY(2px)';
            reserveBtn.innerHTML = '⏳ BOOKING...';
        }
        updateLiveDetails('status', 'API: Booking Slot...'); addLog(`🚀 [Script] Firing Reserve Slot API...`, "warning");

        try {
            const res = await fetch("https://api.ivacbd.com/iams/api/v1/slots/reserveSlot", { method: "POST", headers: { "Accept": "application/json", "Content-Type": "application/json", "Authorization": "Bearer " + rawToken }, body: JSON.stringify({ captchaToken: solvedCaptchaToken }) });
            const data = await res.json().catch(()=>({}));
            if (res.ok && (data.successFlag === true || data.status === "OK" || data.status === "OK_EXISTING" || data.status === "OK_NEW" || data.reservationId)) {
                let paymentAmt = 1500;
                try {
                    const payRes = await fetch("https://api.ivacbd.com/iams/api/v1/file/payment-amount", { method: "GET", headers: { "Accept": "application/json, text/plain, */*", "Authorization": "Bearer " + rawToken } });
                    const payData = await payRes.json().catch(()=>({}));
                    if (payRes.ok && payData.successFlag && payData.data) {
                        paymentAmt = payData.data;
                        addLog(`💰 [API] Payment Fetched: ${paymentAmt} BDT`, "success");
                    }
                } catch(e) {}
                if (data.reservationId) {
                    localStorage.setItem('reservationId', data.reservationId);
                    sessionStorage.setItem('reservationId', data.reservationId);
                    if(data.appointmentDate) sessionStorage.setItem('appointmentDate', data.appointmentDate);
                    sessionStorage.setItem('paymentAmount', paymentAmt);
                    try {
                        ...
                        for(let key of storageKeys) {
                            if(key.includes('storage')) {
                                ...
                                if(parsed && parsed.state) {
                                    parsed.state.reservationId = data.reservationId;
                                    if(data.appointmentDate) parsed.state.appointmentDate = data.appointmentDate;
                                    parsed.state.paymentAmount = paymentAmt;
                                    parsed.state.amount = paymentAmt;
                ...
                updateLiveDetails('status', 'BOOKED! See Popup.');
                addLog("✅ [API] BOOKING SUCCESS! Slot is secured.", "success");
                showToast("Success", "Slot Booked Successfully!", "success");
                automationActive = false;
                sessionStorage.setItem('ivac_auto_active', 'false');
                let autoBtn = document.getElementById('btn-toggle-auto');
                if (autoBtn) {
                    autoBtn.innerText = "▶ RESUME ALL";
                    autoBtn.className = "btn-glossy g-green";
                }
                showSuccessPopup(data.reservationId);
            } else if (res.status === 503 || data.statusCode === 503) {
                if (!automationActive) { isReservingApi = false; addLog("🛑 Auto-Retry cancelled.", "warning"); return; }
                let delay = fastHitActive ? (Math.floor(Math.random() * (SETTINGS.apiMaxDelay - SETTINGS.apiMinDelay + 1)) + SETTINGS.apiMinDelay) : (Math.floor(Math.random() * (SETTINGS.fastMax - SETTINGS.fastMin + 1)) + SETTINGS.fastMin);
                updateLiveDetails('status', `503 Busy - Delay ${(delay/1000).toFixed(1)}s`); setTimeout(() => { isReservingApi = false; }, delay);
            } else {
                if (!automationActive) { isReservingApi = false; addLog("🛑 Auto-Retry cancelled.", "warning"); return; }
                let randomErrorDelay = Math.floor(Math.random() * (4000 - 2000 + 1)) + 2000; updateLiveDetails('status', 'Booking Failed - Retrying'); setTimeout(() => { isReservingApi = false; }, randomErrorDelay);
            }
        } catch (e) {
            updateLiveDetails('status', 'Network Error');
            if (!automationActive) { isReservingApi = false; return; }
            setTimeout(() => { isReservingApi = false; }, 2000);
        } finally {
            if (reserveBtn) {
                reserveBtn.disabled = false;
                reserveBtn.style.opacity = '1';
                reserveBtn.style.transform = '';
                reserveBtn.innerHTML = origText;
            }
        }
    }
    ...
    function renderApp() {
        ...
        panel.innerHTML = `
            <div class="ui-header">
                <div style="display:flex; align-items:center;">
                    <span id="token-dot" class="dot"></span>
                    <span style="letter-spacing: 0.5px;">SMARTIT IVAC_2.0 <sup style="color: #fde047; font-weight: 900;">@S</sup></span>
                </div>
                <button id="close-btn" title="Close Panel">✖</button>
            </div>
            <div class="ui-body">
                <div style="display: flex; gap: 8px; margin-bottom: 6px; flex-shrink: 0; align-items: center;">
                    <input type="text" id="ui-phone" class="ui-input-small" value="${SETTINGS.phone}" placeholder="Phone">
                    <input type="password" id="ui-pass" class="ui-input-small" value="${SETTINGS.pass}" placeholder="Password">
                </div>
                <div style="display: flex; gap: 8px; margin-bottom: 10px; flex-shrink: 0; align-items: center;">
                    <input type="text" id="ui-capsolver" class="ui-input-small" style="flex: 3; border-color: #8b5cf6;" value="${SETTINGS.capKey}" placeholder="CapSolver API Key">
                    <button class="ui-btn btn-blue" id="btn-save-creds" style="margin:0; padding:10px 8px; flex: 1; font-size: 11px; min-width: 60px; background: #3b82f6; color: white; border: none; border-radius: 8px; font-weight: bold; cursor: pointer; transition: 0.2s;">💾 SAVE</button>
                </div>
                <div id="live-details-box">
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px 12px; border-top: 1px dashed rgba(16,185,129,0.2); padding-top: 10px;">
                        ...
                        <div style="display:flex; justify-content:space-between; align-items:center; min-width:0;"><span style="font-weight:600; opacity:0.8; font-size:11px; margin-right:4px; white-space:nowrap;">Slots:</span><span class="detail-value" id="detail-slots" style="color:#dc2626;">Search...</span></div>
                ...
                <div style="text-align:center; margin-top: 10px; font-size: 10px; font-weight: bold; color: #64748b; letter-spacing: 1px;">
                    © SMARTIT <span style="color:#10b981;">@S</span> | MD YEASIR SHARAFAT
                </div>
            </div>
        `;
        document.body.appendChild(panel);
        if (savedPanelState.w) panel.style.width = savedPanelState.w;
        if (savedPanelState.h) panel.style.height = savedPanelState.h;
        if (savedPanelState.t) panel.style.top = savedPanelState.t;
        if (savedPanelState.l) { panel.style.left = savedPanelState.l; panel.style.right = 'auto'; }
        setupCustomResize(panel); setupDrag(panel);

        document.getElementById('close-btn').addEventListener('click', (e) => { e.preventDefault(); e.stopPropagation(); panel.remove(); });

        document.getElementById('btn-clear-log').onclick = () => {
            document.getElementById('live-log').innerHTML = '';
            sessionStorage.removeItem('ivac_live_logs');
            apiHitCount = 0;
            sessionStorage.setItem('ivac_api_hits', 0);
            updateLiveDetails('api-hits', '0');
            memoryLastDate = "";
            memoryLastTime = "";

            if (activeGlowBtnId) {
                let btn = document.getElementById(activeGlowBtnId);
                if (btn) btn.classList.remove('btn-highlight');
                activeGlowBtnId = null;
            }
        ...
        document.getElementById('btn-save-creds').onclick = () => {
            const phone = document.getElementById('ui-phone').value.trim();
            const pass = document.getElementById('ui-pass').value.trim();
            const capKey = document.getElementById('ui-capsolver').value.trim();
            if (phone && pass) {
                localStorage.setItem('ivac_phone', phone);
                localStorage.setItem('ivac_pass', pass);
                localStorage.setItem('ivac_capsolver_key', capKey);
                SETTINGS.phone = phone; SETTINGS.pass = pass; SETTINGS.capKey = capKey;
                addLog("💾 Credentials & CapSolver Key Saved!", "success");
            }
        };

        document.getElementById('btn-force-signin').onclick = pureApiSignIn;
        document.getElementById('btn-toggle-fast').onclick = function() {
            fastHitActive = !fastHitActive;
            sessionStorage.setItem('ivac_fast_active', fastHitActive);
            this.innerText = fastHitActive ? "⏸ FAST" : "▶ FAST";
            this.className = fastHitActive ? "btn-glossy g-cyan" : "btn-glossy g-green";
        };
        document.getElementById('btn-toggle-auto').onclick = function() {
            automationActive = !automationActive;
            sessionStorage.setItem('ivac_auto_active', automationActive);
            if (!automationActive) {
                this.innerText = "▶ RESUME ALL";
                this.className = "btn-glossy g-green";
                updateLiveDetails('status', 'Stopped All');
                addLog("🛑 STOP ALL activated. Auto-retries and hits disabled.", "warning");
                clearTimeout(signInRetryTimeout);
                clearTimeout(otpRetryTimeout);
                isReservingApi = false;
            } else {
                this.innerText = "🛑 STOP ALL";
                this.className = "btn-glossy g-red";
                updateLiveDetails('status', 'Running');
                addLog("▶ RESUME ALL activated.", "success");
            }
        };
        ...
        checkAuthStatus();
        updateLiveDetails('status', automationActive ? 'Running' : 'Stopped');
    ...
    const runEngine = () => {
        ...
        const phoneInput = document.querySelector('input[placeholder*="01"]') || document.querySelector('input[name="loginId"]') || document.querySelector('input[type="tel"]');
        const passInput = document.querySelector('input[type="password"]');
        if (phoneInput || passInput) {
            const savedPhone = document.getElementById('ui-phone')?.value || SETTINGS.phone;
            const savedPass = document.getElementById('ui-pass')?.value || SETTINGS.pass;
            if (phoneInput && savedPhone) setReactInputValue(phoneInput, savedPhone);
            if (passInput && savedPass) setReactInputValue(passInput, savedPass);
        }
        if (path.includes('/signin') || path.includes('/verify-login-phone-otp')) return;
        document.querySelectorAll('svg.lucide-x').forEach(svg => {
            const btn = svg.closest('button');
            if (btn && !btn.dataset.clicked && !btn.closest('#ivac-toast-container')) {
                const isToast = btn.closest('ol') || (btn.className && btn.className.includes('toast'));
                if (!isToast) {
                    btn.dataset.clicked = "true";
                    btn.click();
                    addLog("🧹 Auto-closed a popup/toast", "info");
                }
            }
        });
        if (path.includes('/time-slot')) {
            autoSelectDateTime();
            let cToken = document.querySelector('[name="g-recaptcha-response"]')?.value || document.querySelector('input[name="captchaToken"]')?.value;
            // Either we have manual token, OR let it pass without token so CapSolver kicks in
            if (cToken || document.getElementById('ui-capsolver').value.trim()) {
    ...
    const unlockEverything = () => {
        const h = (e) => {
            if (e.target && typeof e.target.closest === 'function' && (e.target.closest('#ivac-hybrid-panel') || e.target.closest('#success-payment-popup') || e.target.closest('#ivac-toast-container'))) {
                return;
            }
            e.stopPropagation();
        };
        ['contextmenu', 'copy', 'paste', 'selectstart'].forEach(v => {
            window.addEventListener(v, h, true); document.addEventListener(v, h, true);
        });
    };
